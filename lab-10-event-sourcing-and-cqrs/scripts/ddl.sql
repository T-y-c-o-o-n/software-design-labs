...
(
    client_id int         not null autoincrement,
...
(
    subscription_id int not null autoincrement,
...
--  однако его действие длится в днях и нет смысла хранить в поле until дату и время.
    ...
    constraint SubscriptionEvents_PK primary key (subscription_id, when),
...
--  Считаю, что турникет пропускает абонемент (пластиковую карту например, или штрих код в приложении)
    ...
    constraint SubscriptionEvents_P1 primary key (subscription_id, when),
    constraint SubscriptionEvents_FK1 foreign key (subscription_id) references Subscriptions (subscription_id)
);
...
create index SubscriptionEvents_SId_When on SubscriptionEvents using btree (subscription_id, when);
create index TurnstileEvents_SId_When on TurnstileEvents using btree (subscription_id, when);
